describe('Usuário logado na página de dashboard', () => {
    ...
    context('Modal de cadastro de especialista', () => {
        ...
        it('Seleciona o btão checkbox "Atende por plano?" para visualizar os planos de saúde', () => {
            cy.visit('/dashboard');
            cy.contains('Cadastrar especialista').should('be.visible').click()
            cy.get('[type="checkbox"]').should('have.attr', 'aria-label', 'Atende por plano?').and('not.be.checked').check()
